---
format: revealjs
---

## Today's Agenda {background-image="Images/00-Leviathan_Cover_55.png" .center}

```{r}
library(tidyverse)
library(readxl)
library(kableExtra)
```

<br>

**II. How and why do governments use violence against the people inside their borders?**

::: {.r-fit-text}

- Case studies of political violence by state governments

:::

<br>

::: r-stack
Justin Leinaweaver (Fall 2025)
:::

::: notes
Prep for Class

1. Review Canvas submissions

2. Save the class hypotheses from the end of work today in next class slides

<br>

Over the next two weeks we will dive deeply into the challenge of measuring political violence at the domestic level.

<br>

FIRST, thinking carefully about measurement requires both broad and deep explorations of a concept

- That means that our work should dramatically deepen your expertise in the concept of political violence

<br>

SECOND, each data project represents a group of experts in the field trying to convince you that their measurement of political violence is useful

- This means our review should rapidly bring us all up to speed on the current debates at the cutting-edge of the discipline

<br>

FINALLY, these data projects will serve as the basis of the first two papers you will write for this class.

- So, let's make sure to dig into them as a group before you have to use them on your own!

<br>

**SIDE**: Course outline big picture

:::



## Section 2 {background-image="Images/00-Leviathan_Cover_55.png" .center}

<br>

### **How and why do governments use violence against the people inside their borders?**

::: notes

All of this work remains in service to answering the motivating question in Section 2 of our class.

<br>

### What was Weber's answer to this question?
- (The state is a "compulsory association which organizes domination")
    - e.g. The state IS violence
    
<br>

In terms of understanding the model:

### What were the key interests, institutions and interactions that lead to this conclusion?

- (**SLIDE**)

:::



## Weber's (1918/1946) Model {background-image="Images/background-desert_rock-Filtered50.png" .center}

**Interests**

- Politicians want to [**expropriate**]{style="color:red;"} value and increase their [**power**]{style="color:red;"}

**Institutions**

- The state [**monopolizes**]{style="color:red;"} the [**legitimate use of force**]{style="color:red;"}

**Interactions**

- To maintain control, leaders must ensure mass [**obedience**]{style="color:red;"} from the people and the administrative state

::: notes

According to Weber, the state IS violence and that conclusion is based on a fairly simple three part model

1. Interests: THIS is a model focused on political leaders trying to extract value and power from the state.

2. Institutions: The rules are set by the state which can essentially justify any action by the leader

3. Interactions: The only obstacles to the leader's ability to get what he wants is ensuring the obedience of the people and the administrative state.

<br>

### Does this model have any problem with our class's fundamental assumption about political violence being a rational, strategic tool? Why or why not?

<br>

### And how does Olson modify Weber's model of domestic violence by governments?

### - e.g. interests, institutions and interactions?
- (The state organizes violence to productive ends)

- (**SLIDE**)    

:::



## {background-image="Images/background-desert_rock-Filtered50.png" .center}

**Interests**

- Individuals -> peaceful order and public goods
- Leader -> profit-maximization

**Institutions**

- Government sets tax rates and provides goods

**Interactions**

- Dictators provide some public goods at very high cost assuming long time horizons
- Democratic governments provide public goods at lower cost assuming broad representation

::: notes

In short, Olson's model focuses on two actors, the rule-making authority of the government and the differences in incentives by regime type.

<br>

### Does this model have any problem with our class's fundamental assumption about political violence being a rational, strategic tool? Why or why not?

<br>

According to Olson's model, explaining the variation in safety and public goods in society requires thinking about:

1. Interests: The interplay between leaders who want to profit and the public who want peace and prosperity

2. Institutions: The rules are still set by the government

3. Interactions: 
    - Long-term profit is impossible if the people don't feel safe and provided for
    - But different types of government prioritize different kinds of policies
    
So, the state organizes violence to productive ends!

<br>

Some of what I hope you got from our work last week:

1. Scientific models are designed to help us explain and predict what happens in the real world

2. Scientific models are simplifications that allow us to focusin on specific causal mechanisms

3. Being this clear in our model building encourages growth of knowledge over time

    - Olson is 100% iterating and improving on Weber's model by making clear:

    1) how the people play a role in the system, and

    2) how different regime types react to those people differently
    
<br>

**Any questions on the models from last week?**

:::



## For Today {background-image="Images/00-Leviathan_Cover_55.png" .center}

<br>

Find us TWO real world examples of state governments using violence against the people inside their territory.

- One from a democracy and one from a dictatorship

- Must be specific events (e.g. action X on date Y)

- No overlap!

::: notes

**Has everybody submitted their TWO cases?**

<br>

**Are all of the citations in APA format?**

- *Do a quick check?*

<br>

<br>

Canvas Discussion: Governments Using "Political Violence"

Find us TWO real world examples of state governments using violence against the people inside their territory. One example should come from a country you consider to be more "democratic" and one from a country you consider to be more "autocratic." Your examples should be specific events (e.g. action X on date Y) rather than broad generalizations (country X harms group Y). No overlap in events OR countries!

For EACH example submit the following:

1. Country name
2. Is this the more "democratic" or "autocratic" example? Explain your categorization (2+ sentences)
3. APA citation to the event
4. A brief explanation of the event you selected and why you selected it (3+ sentences)

:::



## The "More Democratic" Cases {background-image="Images/00-Leviathan_Cover_55.png" .center}

![](Images/04_1-voting.jpg){style="display: block; margin: 0 auto"}

::: notes

We'll kick things off by focusing on your democracy cases.

- Everybody take a few minutes to review ALL of the DEMOCRACY cases submitted on Canvas for today

- Reflect on the countries selected, the violence exhibited and the reasons provided.

<br>

Ok, help me to unpack these cases (e.g. prepare them for analysis)

- *ON BOARD* (split board in 1/2: Democ vs Autoc)

- *SAVE THESE NOTES!*

<br>

Column 1: What characterizes these states as "more democratic"?

<br>

**Based on these selections, what are the qualities our class prioritized as important for democracy?**

<br>

Column 2: Who specifically was doing the violence?

<br>

Column 3: What specific "types" of violence were done?

<br>

Column 4: Who was targeted by these acts?

<br>

*Split class into groups of 3-4*

- Go sit with your group

:::



## {background-image="Images/00-Leviathan_Cover_55.png" .center}

**When do we predict that democratic governments will use violence against the people in their territory?**

![](Images/04_1-voting.jpg){style="display: block; margin: 0 auto"}

::: notes

Groups take some time to reflect on all of this data in order to brainstorm a few explanatory mechanisms

- In other words, develop a few hypotheses based on these cases

<br>

PRESENT and DISCUSS

<br>

*SAVE THESE NOTES!*

<br>

*Form new groups!* **DO THIS!**

- Go sit with your NEW group

:::



## {background-image="Images/06_2-autocrats_playbook.jpg" .center}

::: notes

Now we'll switch to the autocratic cases.

- Everybody take a few minutes to review ALL of the AUTOCRACY cases submitted on Canvas for today

- Reflect on the countries selected, the violence exhibited and the reasons provided.

<br>

Ok, help me to unpack these cases (e.g. prepare them for analysis)

- *ON BOARD* (split board in 1/2: Democ vs Autoc)

- *SAVE THESE NOTES!*

<br>

Column 1: What characterizes these states as "more autocratic"?

<br>

**Based on these selections, what are the qualities our class prioritized as important for autocracy?**

<br>

Column 2: Who specifically was doing the violence?

<br>

Column 3: What specific "types" of violence were done?

<br>

Column 4: Who was targeted by these acts?

<br>

**SLIDE**: Groups prompt

:::



## {background-image="Images/00-Leviathan_Cover_55.png"}

**When do we predict that autocratic governments will use violence against the people in their territory?**

![](Images/06_2-autocrats_playbook.jpg){style="display: block; margin: 0 auto"}

::: notes

Groups take some time to reflect on all of this data in order to brainstorm a few explanatory mechanisms

- In other words, develop a few hypotheses based on these cases

<br>

PRESENT and DISCUSS

<br>

*SAVE THESE NOTES!*

<br>

**SLIDE**: Let's wrap up our work today with some big picture questions

:::



## Section 2 {background-image="Images/00-Leviathan_Cover_55.png" .center}

<br>

### **How and why do governments use violence against the people inside their borders?**

<br>

- Q1) Is domestic government violence strategic or existential?

::: notes

**At this point, are you convinced that more useful models treat violence as a strategy for states or as fundamental to their existences?**

- *Force discussion*

:::



## Section 2 {background-image="Images/00-Leviathan_Cover_55.png" .center}

<br>

### **How and why do governments use violence against the people inside their borders?**

<br>

- Q2) Do our models of domestic government violence need to be split by regime type?

::: notes

**At this point, are you convinced that we need different models to explain the behavior of democracies vs the behavior of dictatorships? Why or why not?**

- *Force discussion*

:::



## {background-image="Images/00-Leviathan_Cover_55.png" .center}

::: {.r-fit-text}
**Sources of Political Violence Data**
:::

- The US State Department's "Country Reports on Human Rights Practices"

- Amnesty International's "Annual Country Reports"

- The Political Terror Scale (PTS)

- The CIRIGHTS data project's "Physical Integrity Rights"

- Varieties of Democracy's (V-Dem) "Personal Integrity Rights"

::: notes

Our work today represents an excellent warm-up, but isn't nearly sufficient to give us a good sense of what political violence by governments actually looks like in the world.

- *Save reasons "why" for next class*

<br>

Over the next two weeks we will examine these five sources of data on government political violence.

- Each is built on a much wider array of cases and expert evaluations.

- These also represent some of the most commonly utilized measures of domestic political violence in the academic literature

<br>

**SLIDE**: This work will lead directly to your first paper!

:::



## Paper 1 {background-image="Images/00-Leviathan_Cover_55.png" .center}

<br>

Which **SINGLE** data source would you recommend to someone interested in better understanding the use of political violence by governments around the world?

::: {.incremental}

- Your thesis should be a clear recommendation for **ONE** of the sources and should be supported by **AT LEAST THREE distinct reasons** for your recommendation

- **A high quality recommendation** will also make clear **why** you are not recommending the other sources

::: 

::: notes

**REVEAL x 2**

<br>

**Questions on the prompt?**

- On Canvas now.

<br>

**SLIDE**: Next class

:::



## For Next Class {background-image="Images/00-Leviathan_Cover_55.png" .center}

<br>

### Analyzing the US State Department Human Rights Reports

::: notes

Next class we focus on our first source

- This is an exercise some of you may have done for me in the past!

<br>

The readings are meant to help us build on the work we've done today AND to think critically about the validity and reliability of these reports as data.

- The syllabus also mentions an optional backgrounder on the reports from the Congressional Reporting Service (Weber 2023).

<br>

Your case analyses for next class:

- Ideally, you should keep working on the two countries you analyzed in class today!

- No overlap in country selections!

- Can't do the US

- Pick one "democracy" and one "dictatorship"

- Submit your analyses to Canvas discussion board

<br>

**If you decide to change countries from today make sure to register that change on our shared Google spreadsheet (in Canvas Modules)**

<br>

**Questions on the assignment?**

<br>

Canvas Discussion: Analyzing the US State Department Human Rights Reports

1. Read Appendix A of the Country Reports on Human Rights Practices (2023), Nordas (2021) and Smith (2025)

2. Select two countries (non-US) and read the Country Reports on Human Rights Practices (State Department) for both 2022 and 2023. (2 countries x 2 years = 4 reports). Ideally, try to select one of the countries you analyzed on Monday. 

    1. What countries did you analyze? (No overlap!)
    
    2. Briefly summarize the state of human rights in each country from 2022 to 2023 (2-3 sentences each).


:::

