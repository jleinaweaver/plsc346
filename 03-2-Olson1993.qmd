...
Ok, where do we stand on this model?

1. **Is it logical?**

2. **When does it make good predictions? Examples?**

3. **And when does it make bad predictions? Examples?**

<br>

**On balance, is Weber's model convincing as a map of state political violence? Why or why not?**

<br>

My problem with Weber is that this model predicts violence (expropriation or physical harm) in basically every circumstance.
...
d1 <- read_excel("../../Data/V-Dem/2025-Fall-Class/V_Dem_High_Level_Democracy_Indices.xlsx", na = "NA") |>
  filter(year == 2023) |>
  select(country_name:v2x_polyarchy)

d2 <- read_excel("../../Data/WDI/WDI-GDP-2023_2025-08-20.xlsx", na = "NA") |>
  select(year = Time,
...
d <- right_join(d1a, d2a, by = c("country_text_id" = "country_code"))
...
d10 |>
  ...
  labs(fill = "", title = "GDP per capita (2023 USD)") +
...
Here we see a map of GDP per capita levels in 2023 (most recently published by WB; 2024 is still incomplete)
...
**Is this map's results consistent with your prior knowledge / intuitions about the countries of the world?**
...
d10 |>
  ...
  labs(fill = "", title = "Polyarchy Scores (2023)",
...
**Is this map's results consistent with your prior knowledge / intuitions about the countries around the world?**
...
p1 <- ggplot(data = d, aes(x = v2x_polyarchy, y = gdp_pc)) +
  ...
  theme_bw() +
  labs(x = "V-Dem Polyarchy (2023)", y = "GDP per capita (2023 USD)")
...
Find us TWO real world examples of state governments using violence against the people inside their territory.

- One from a democracy and one from a dictatorship

- Must be specific events (e.g. action X on date Y)
...
Canvas Discussion: Governments Using "Political Violence"

Find us TWO real world examples of state governments using violence against the people inside their territory. One example should come from a country you consider to be more "democratic" and one from a country you consider to be more "autocratic." Your examples should be specific events (e.g. action X on date Y) rather than broad generalizations (country X harms group Y). No overlap in events OR countries!

For EACH example submit the following:

1. Country name
2. Is this the more "democratic" or "autocratic" example? Explain your categorization (2+ sentences)
3. APA citation to the event
4. A brief explanation of the event you selected and why you selected it (3+ sentences)
...
d1 |>
  ...
  write_excel_csv("2023-polyarchy.csv")
