...
Let's check out the recent five years of the data.
...
```{r, fig.retina=3, fig.align='center', out.width='100%', fig.height=5, fig.width=8}
###################################################
## Map of terror attacks per country (2015-2019)
d |>
  filter(iyear >= 2015) |>
  group_by(ccode) |>
  summarize(
  ...
  mutate(
    Attacks_cat = cut(Attacks, breaks = c(0, 2, 20, 200, 11000), labels = c("< 2", "< 20", "< 200", "> 200"), include.lowest = TRUE)
  ) %>%
  left_join(worldmap, ., by = c("sovereignt" = "ccode")) |>
  ggplot() +
  geom_sf(aes(fill = Attacks_cat)) +
  labs(fill = "", title = "Terror Attacks (2015 - 2019)",
       caption = "Source: GTD") +
  scale_fill_brewer(type = "seq", palette = "YlOrRd")
```
...
```{r, fig.retina=3, fig.align='center', out.width='100%', fig.height=5, fig.width=8}
###################################################
## Map of terror fatalities per country (2015-2019)
d |>
  filter(iyear >= 2015) |>
  group_by(ccode) |>
  summarize(
  ...
  mutate(
    Fatal_cat = cut(Fatal, breaks = c(0, 5, 25, 250, 37000), labels = c("< 5", "< 25", "< 250", "> 250"), include.lowest = TRUE)
  ) %>%
  left_join(worldmap, ., by = c("sovereignt" = "ccode")) |>
  ggplot() +
  geom_sf(aes(fill = Fatal_cat)) +
  labs(fill = "", title = "Terrorism Fatalities (2015 - 2019)",
       caption = "Source: GTD") +
  scale_fill_brewer(type = "seq", palette = "YlOrRd")
```
