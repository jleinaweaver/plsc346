...
d10 <- left_join(worldmap, d2020_1, by = c("un_a3" = "newcode2"))

d10 |>
  mutate(
    Matching_cat = if_else(Matching == 1, "Matching", "Not Matching", missing = "No Data"),
    Matching_cat = factor(Matching_cat, levels = c("Matching", "Not Matching", "No Data"))
  ) |>
...
### Evidence of a selection effect in the states that match vs those that don't?
...
d2014_1$newcode2 <- countrycode::countrycode(d2014_1$COW_Code_A, origin = "cowc", destination = "genc3n")
...
## Merge data onto worldmap
d10 <- left_join(worldmap, d2014_1, by = c("un_a3" = "newcode2"))
...
d10 |>
  ...
  ggplot() +
  geom_sf(aes(fill = Matching_cat)) +
  labs(fill = "", title = "PTS_A vs PTS_S (2014)") +
  theme(legend.position = "bottom") +
  scale_fill_manual(values = c("gold1", "green4", "white"))
```
...
```{r, fig.retina=3, fig.align='center', out.width='100%', fig.height=5, fig.width=8}
## Make a map
d2020$newcode2 <- countrycode::countrycode(d2014_1$COW_Code_A, origin = "cowc", destination = "genc3n")
...
## Merge data onto worldmap
d10 <- left_join(worldmap, d2020, by = c("un_a3" = "newcode2"))
